...
BUILD_OPTIONS = BuildOptions(project_root=PROJECT_ROOT)


def test_benge_project_builds_and_reconciles() -> None:
    result = build_project(BUILD_OPTIONS)
    generated = PROJECT_ROOT / "generated"

    design = json.loads((generated / "manifests/design-manifest.json").read_text(encoding="utf-8"))
    ...
    assert result.design_semantic_hash == build["design_semantic_hash"]
    assert len(ids) == 236
    assert build["project"]["repository"] == "benge_freecad_project"
    assert "revision" in build["project"]
    assert build["validation"]["status"] == "passed"
    assert design["metadata"]["project"] == "Benge Property CAD"
    assert {
    ...
    step_path = generated / "step/BengeProperty.step"
    ...
    ifc = ifcopenshell.open(generated / "ifc/BengeProperty.ifc")
    ...
    assert {row["element_id"] for row in quantities["records"]} == ids
    assert all(row["volume_mm3"] > 0 for row in quantities["records"])
    ...
    plan = ET.parse(generated / "drawings/svg/BengeProperty_plan.svg").getroot()
    ...
    pdf = PdfReader(generated / "drawings/pdf/BengeProperty_Conceptual_Drawings.pdf")
