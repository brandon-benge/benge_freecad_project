...
|---|---|---|
| `benge-design-maintainer` | Implement and test property-design changes | `config.py`, `model.py`, `drawing_annotations.py`, `tests/` |
| `benge-artifact-reviewer` | Review generated outputs and report design-quality findings | Nothing |
...
- `benge-design-maintainer` may call `benge-artifact-reviewer` to evaluate
  regenerated outputs.
- `benge-design-maintainer` may call `cad-compatibility-verifier` to validate the
  installed package, environment, or declared project checks.
- `benge-artifact-reviewer` may call `benge-design-maintainer` when its evidence
  requires source changes.
- `benge-artifact-reviewer` may call `cad-compatibility-verifier` when an
